...
test('consumer.close() succeeds', async () => {
    ...
    await expect(audioConsumer.dump()).rejects.toThrow(errors_1.NotFoundError);
...
test('Consumer methods reject if closed', async () => {
    ...
    audioConsumer.close();
    await expect(audioConsumer.dump()).rejects.toThrow(errors_1.NotFoundError);
    await expect(audioConsumer.getStats()).rejects.toThrow(errors_1.NotFoundError);
    await expect(audioConsumer.pause()).rejects.toThrow(errors_1.NotFoundError);
    await expect(audioConsumer.resume()).rejects.toThrow(errors_1.NotFoundError);
    // @ts-expect-error --- Testing purposes.
    await expect(audioConsumer.setPreferredLayers({})).rejects.toThrow(TypeError);
    await expect(audioConsumer.setPriority(2)).rejects.toThrow(errors_1.NotFoundError);
    await expect(audioConsumer.requestKeyFrame()).rejects.toThrow(errors_1.NotFoundError);
}, 2000);
...
test('Consumer emits "transportclose" if Transport is closed', async () => {
    ...
    await expect(videoConsumer.dump()).rejects.toThrow(errors_1.NotFoundError);
