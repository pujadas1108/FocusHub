...
test('producer.close() succeeds', async () => {
    ...
    await expect(audioProducer.dump()).rejects.toThrow(errors_1.NotFoundError);
    await expect(ctx.router.dump()).resolves.toMatchObject({
...
test('Producer methods reject if closed', async () => {
    ...
    audioProducer.close();
    await expect(audioProducer.dump()).rejects.toThrow(errors_1.NotFoundError);
    await expect(audioProducer.getStats()).rejects.toThrow(errors_1.NotFoundError);
    await expect(audioProducer.pause()).rejects.toThrow(errors_1.NotFoundError);
    await expect(audioProducer.resume()).rejects.toThrow(errors_1.NotFoundError);
}, 2000);
test('Producer emits "transportclose" if Transport is closed', async () => {
    ...
    await expect(videoProducer.dump()).rejects.toThrow(errors_1.NotFoundError);
}, 2000);
