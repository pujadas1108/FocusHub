...
test('transport.consumeData() with unknown dataProducerId fails', async () => {
    await expect(ctx.webRtcTransport2.consumeData({
...
test('dataConsumer.close() succeeds', async () => {
    ...
    await expect(dataConsumer.dump()).rejects.toThrow(errors_1.NotFoundError);
    const dump = await ctx.router.dump();
...
test('Consumer methods reject if closed', async () => {
    ...
    dataConsumer.close();
    await expect(dataConsumer.dump()).rejects.toThrow(errors_1.NotFoundError);
    await expect(dataConsumer.getStats()).rejects.toThrow(errors_1.NotFoundError);
}, 2000);
